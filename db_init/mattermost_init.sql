-- Mattermost: Create scoped agent users with limited permissions
-- This runs on first DB initialization via docker-entrypoint-initdb.d

-- Read-only agent user
CREATE ROLE agent_ro LOGIN PASSWORD 'agent_mm_pass';
GRANT CONNECT ON DATABASE mattermost TO agent_ro;
GRANT USAGE ON SCHEMA public TO agent_ro;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO agent_ro;

-- Read-write agent user (for experiment)
CREATE ROLE agent_rw LOGIN PASSWORD 'agent_mm_pass';
GRANT CONNECT ON DATABASE mattermost TO agent_rw;
