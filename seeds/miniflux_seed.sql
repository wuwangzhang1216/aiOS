...
-- Miniflux creates an admin user on first boot.
-- We add categories and feeds for the experiment.
...
-- ── Entries ──
-- Sample entries for experiment scenarios
...
CROSS JOIN LATERAL (VALUES
    ('Hacker News', 'Show HN: A new approach to database migrations',
     'https://example.com/db-migrations', '<p>A new tool for zero-downtime schema changes</p>',
     'alice_dev', 2),
    ('Hacker News', 'Why SQLite is the most deployed database',
     'https://example.com/sqlite-deployed', '<p>SQLite handles most use cases better than you think</p>',
     'bob_eng', 5),
    ('Go Blog', 'Go 1.23 Release Notes',
     'https://blog.golang.org/go1.23', '<p>New features in Go 1.23 including range-over-func</p>',
     'Go Team', 12),
    ('Kubernetes Blog', 'Kubernetes v1.31 release',
     'https://kubernetes.io/blog/k8s-131', '<p>New features in Kubernetes 1.31</p>',
     'K8s Release Team', 24)
